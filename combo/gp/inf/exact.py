...
def get_grad_marlik(gp, X, t, params=None):
    ...
    if gp.lik.num_params != 0:
        ...
        grad_marlik[0:gp.lik.num_params] \
            = -0.5 * temp.dot(alpha) + 0.5 * misc.traceAB2(invA, lik_grad)
    ...
    if gp.prior.mean.num_params != 0:
        ...
        grad_marlik[ntemp:ntemp+gp.prior.mean.num_params] \
            = - np.inner(alpha, mean_grad)
    ...
    if gp.prior.cov.num_params != 0:
        ...
        grad_marlik[ntemp:] = -0.5 * temp.dot(alpha)
        grad_marlik[ntemp:] += 0.5 * misc.traceAB3(invA, cov_grad)
