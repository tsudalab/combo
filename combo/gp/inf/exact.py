...
def eval_marlik( gp, X, t, params = None ):
    ndata, ndims = X.shape
    lik_params, prior_params = gp._decomp_params( params )
...
def get_grad_marlik( gp, X, t, params = None ):
    ...
    grad_marlik = np.zeros( gp.nparams )

    ''' lik '''
    if gp.lik.nparams !=0:
        lik_grad = gp.lik.get_grad( ndata, lik_params )
        temp = lik_grad.dot( alpha )
        grad_marlik[0:gp.lik.nparams] = -0.5 * temp.dot(alpha) + 0.5 * misc.traceAB2( invA, lik_grad )

    ntemp = gp.lik.nparams
    ''' prior '''
    if gp.prior.mean.nparams !=0:
        mean_grad = gp.prior.get_grad_mean( ndata, prior_params )
        grad_marlik[ntemp:ntemp+gp.prior.mean.nparams] = - np.inner(alpha, mean_grad)

    ntemp += gp.prior.mean.nparams

    if gp.prior.cov.nparams != 0:
...
def comp_stats( gp, X, t, params = None ):
    ndata = X.shape[0]
    ndims = X.shape[1]
